 ...
 * withCallState
 *
 * Reusable call-state pattern for the AI lib.
 * Identical contract to the billing lib's withCallState — portable and
 * self-contained within this lib.
 *
 * Usage:
 *   withCallState('chat')
 *   → state:   chatLoading, chatSuccess, chatError
 *   → methods: chatSetLoading(), chatSetSuccess(),
 *               chatSetError(msg), chatResetState()
 */
  ...
  return signalStoreFeature(
    ...
    withMethods(
      (store): Methods =>
        ({
          [`${name}SetLoading`]: () => {
            patchState(store as WritableStateSource<State>, {
              [`${name}Loading`]: true,
              [`${name}Success`]: false,
              [`${name}Error`]: null,
            } as Partial<State>);

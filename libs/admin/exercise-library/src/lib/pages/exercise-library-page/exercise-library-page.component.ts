...
import type { DifficultyId } from '../../shared/catalogs';
...
export class ExerciseLibraryPageComponent {
  ...
  /** Returns the Spanish label for a difficulty ID */
  difficultyLabel(id: DifficultyId): string {
    return labelById(DIFFICULTY_CATALOG, id);
  }
  ...
  difficultyTag(
    ...
    | 'contrast'
    | undefined {
    const map: Record<DifficultyId, 'success' | 'warn' | 'danger'> = {
      beginner: 'success',
      intermediate: 'warn',
      advanced: 'danger',
    };
